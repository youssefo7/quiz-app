...
import { Choice } from '@app/interfaces/quiz';
import { Constants } from '@common/constants';
import { GameService } from './game.service';

describe('GameService', () => {
    let service: GameService;
    const question = { type: 'QRL', text: 'qrlTestQuestion', points: 100 };
    const questionQCM = { type: 'QCM', text: 'qcmTestQuestion', points: 100, choices: [{ text: 'Choice 1', isCorrect: true }] };
    ...
    it('should reset points manager correctly', () => {
        const pointsManager = { points: 100, pointsToDisplay: 120, pointsMessage: '(20% bonus Woohoo!)', doesDisplayPoints: true };
    ...
    it('should determine the correct QRL message for 0 points', () => {
        const pointsManager = { points: 0, pointsToDisplay: 0, pointsMessage: '', doesDisplayPoints: false };
    ...
    it('should determine the correct QRL message for maximum points', () => {
        const pointsManager = { points: 100, pointsToDisplay: 100, pointsMessage: '', doesDisplayPoints: false };
        const result = service['determineQRLMessage'](pointsManager, question);
        expect(result).toBe('(100% Bravo!)');
    });

    it('should determine the correct QRL message for other points', () => {
        const pointsManager = { points: 50, pointsToDisplay: 50, pointsMessage: '', doesDisplayPoints: false };
        const result = service['determineQRLMessage'](pointsManager, question);
