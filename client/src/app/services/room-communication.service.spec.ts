...
describe('RoomCommunicationService', () => {
    ...
    it('should process the username when sending POST request with username, room and socket id in body (HttpClient called once)', () => {
        const usernameData = { name: 'name', roomId: 'mockRoomId', socketId: 'mockSocketId' };
        const mockResult = true;

        service.processUsername(usernameData).subscribe({
            next: (response) => {
        ...
        const req = httpMock.expectOne(`${baseUrl}/rooms/name`);
    ...
    it('should return JoinRoomResponse when sending POST request with room and socket id body (HttpClient called once)', () => {
        const joinData = { roomId: 'mockRoomId', socketId: 'mockSocketId' };
        const mockJoinRoomResponse: JoinRoomResponse = {
            roomState: 'mockState',
            quizId: 'mockId',
        };

        service.joinRoom(joinData).subscribe({
            next: (response) => {
        ...
        const req = httpMock.expectOne(`${baseUrl}/rooms/join`);
        expect(req.request.method).toBe('POST');
