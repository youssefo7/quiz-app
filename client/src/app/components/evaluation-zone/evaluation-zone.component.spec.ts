...
describe('EvaluationZoneComponent', () => {
    ...
    it('should push to playerPoints with 0 pointsToAdd when givePoints() is called for a player with a wrong answer', () => {
        component.questionPoints = 10;
        component['grade'] = GradeValues.WrongAnswer;
        component.currentAnswerIndex = 0;
        component.givePoints();
        expect(component['playersPoints'][0]).toEqual({
            name: component.answers[0].name,
            pointsToAdd: 0,
            roomId: component.roomId,
    ...
    it('should send AddPointsToPlayer event and call the correct methods when the current answer index is on the last answer', () => {
        ...
        expect(sendSpy).toHaveBeenCalledTimes(component['playersPoints'].length);
    ...
    it('should listen on AllPlayersSubmitted event and react correctly when the event is received', () => {
        ...
        expect(component.answers).toEqual(mockedAnswers);
    });

    it('should listen on AllPlayersSubmitted event and handle when no player QRL answers are submitted and question type is QRL', () => {
        component.questionType = QTypes.QRL;
        const resetEvaluationZoneSpy = spyOn<any>(component, 'resetEvaluationZone');
        const savePointsStatisticsSpy = spyOn<any>(component, 'savePointsStatistics');
        const enableNextQuestionButtonSpy = spyOn(component.enableNextQuestionButton, 'emit');

        socketHelper.peerSideEmit(GameEvents.AllPlayersSubmitted, null);

        expect(resetEvaluationZoneSpy).toHaveBeenCalled();
        expect(savePointsStatisticsSpy).toHaveBeenCalled();
        expect(enableNextQuestionButtonSpy).toHaveBeenCalled();
    });
